...
`disclean big`（GUI の「大きいもの」タブ）は、`~/Documents` や `~/Downloads` など**ホームの見えるフォルダ**から
大きいものを探します。ここはキャッシュではなく自分のファイルが並ぶので、**既定では 1 件も選ばれません**。
選んだものだけが隔離庫へ移り、既定 7 日は元に戻せます。

```bash
disclean big                       # 200MB 以上のものを大きい順に見る（消しません）
disclean big --min-megabytes 1000  # 1GB 以上だけを見る
disclean big --in ~/Movies         # この場所だけを見る
disclean big --include-library     # ~/Library の中も見る（既定は見ません）
disclean big --move ~/Downloads/big.dmg --yes   # 選んだものだけ隔離庫へ移す
```

一覧はファイルの羅列にしません。**持ち主でまとめて**から出します。

| まとめかた | 例 | 一覧での扱い |
|---|---|---|
| ひとかたまり | `.app` / `.photoslibrary` / `.xcarchive` / `.fcpbundle` | 中へ下りず、入れ物 1 件 |
| 部品・ビルドの置き場 | `node_modules` / `.venv` / `.git` / `DerivedData` / `build` | まとめて 1 件 + 「作り直せるか」の一行 |
| ファイル | 動画・書き出し・ダウンロードした `.dmg` | そのまま 1 件 |

理由は実測です。ふつうの開発機のホームでは、大きいファイルをそのまま並べると上位がほぼ全部
「部品置き場の中身」になり、1 個ずつでは判断も削除もできません。

扱わないもの: ホームのすぐ下に直接置かれたファイル（隔離庫へ移せる範囲の外）、隠しフォルダ、
除外設定した場所、まだ手元に降りていない iCloud のファイル（開かずに飛ばします）。
...
1.0 は「壊さないことを約束できる状態になった」という意味です。

- 削除は必ず隔離庫を経由し、記録できないときは削除しません
- ルールの更新は署名を検証し、消す対象が増える変更はあなたの承認を待ちます
- 秘密・脆弱性・静的解析・アクセシビリティの検査を、push のたびに機械が回します
- 配信中のルールの有効期限を毎週見張り、切れる前に気づける状態にしてあります

見た目や動きは今後も変わりますが、**上の 4 つは 1.x の間ずっと守ります**。
