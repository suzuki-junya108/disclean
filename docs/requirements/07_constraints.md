...
- **Build target**: ユニバーサルバイナリ（arm64 + x86_64）。CLI は `swift build -c release --arch arm64 --arch x86_64`、GUI は `tools/make-app.sh` が SwiftPM の実行ファイルに `Disclean.app` のバンドル構造を被せて生成する（Xcode プロジェクトを持たない。XCUITest はこのため v1.0 では持たず、GUI の検証は実機の手動 QA で行う）
- **Build output**:
  - CLI: `./.build/apple/Products/Release/disclean`（ユニバーサル）/ 単一アーキ時は `./.build/release/disclean`
  - GUI: `./build/Disclean.app`（`tools/make-app.sh` の出力）
...
- **Fonts**: Bricolage Grotesque（Display）/ Zen Maru Gothic（Body）/ Martian Mono（Data）。いずれも OFL 1.1。LP は Google Fonts から読み込む。**GUI は v0.1.0 では同梱せず**、`docs/design-system.md` §3.1 のフォールバック（Hiragino Maru Gothic ProN / SF Mono）で描く。同梱は v0.2.0 で行う（リポジトリに数 MB のフォントを入れる判断を先送りしたため）
