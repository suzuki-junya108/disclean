...
- **Deployment**:
  ...
  - **版に依存しない名前の資産（`Disclean.dmg` / `disclean-macos-universal.tar.gz`）を必ず併せて添付する**。LP や外部の案内は `releases/latest/download/<固定名>` を指すため、版数入りの名前だけを置くと**次のリリースで静かに 404 になる**（2026-08-21 に実際に発生。LP が `Disclean-0.1.0.dmg` を指したまま v0.1.4 を公開し、ダウンロードできなくなった）。
  - リリース直後に `tools/check-download-links.sh` を実行し、LP のリンク・配信元 manifest・tap・公開 LP が実際に 200 を返すことを確かめる。
