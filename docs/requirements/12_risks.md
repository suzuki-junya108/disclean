...
| R-26 | 外部ツールの `prune` / `clean` は「何を消すか」がツールごとに違い、一覧で見せた量と実際に減る量が食い違う | high | **2026-08-21 対応**。実測で `uv cache prune` は 3.1GB のキャッシュから 0 バイトしか消さないことを確認した。パッケージマネージャのキャッシュは外部コマンドに任せず、`pathsFrom` で場所を聞いてディスクリン自身が隔離庫へ移す方式に変更した（v0.2.0）。見せた量＝動かす量になり、7 日間は取り消せる。Owner: suzuki.junya |
| R-27 | `pathsFrom` でツールが返したパスを信じると、ツールの不具合や汚染で意図しない場所を消す | high | 返ってきたパスにも同梱ルールと同一の検証（ホーム配下・深さ 2 以上・システム領域でない・除外に入っていない）を必ず通し、通らなければ `forbidden-root` として対象から外す。AT-005 と結合テストで検証する。Owner: suzuki.junya |
