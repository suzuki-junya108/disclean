...
#
#  【非推奨】このスクリプトは disclean に置き換えられました。
#    brew install suzuki-junya108/disclean/disclean && disclean scan
#  理由: 確認なしで削除し、権限不足などの失敗を握り潰し、記録も残しません。
#  OS が更新されて対象パスが変わっても、静かに空振りするだけで気づけません。
